...
--drop table if exists ldbc.interactive_short_1;
--EXECUTE FUNCTION
QUERY_GRAPH (
    GRAPH => 'ldbc.graph',
    QUERIES => INPUT_TABLES(
        ...
        (SELECT 1 as HOP_ID, 'city' as NODE_LABEL)
    ),
    RINGS => 1,
    --ADJACENCY_TABLE => 'ldbc.interactive_short_1',
    OPTIONS => KV_PAIRS(
...
INNER JOIN ldbc.place as pl
on s.QUERY_NODE2_NAME = CONCAT(pl.pl_name,'_pl')